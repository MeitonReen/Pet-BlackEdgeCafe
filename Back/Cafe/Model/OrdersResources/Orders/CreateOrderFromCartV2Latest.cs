...
{
	public class CreateOrderFromCartV2Latest : HandlerBase
	{
		...
		public CreateOrderFromCartV2Latest(AppSettings appSettings, CafeDatabase cafeDB)
		{
		...
		protected override async Task ExecuteAsync(ChainRequest request)
		{
			...
			_cafeDB.DishesInOrders.AddRange(
				from dishInClientCart in dishesInClientCart
				select new DishesInOrder()
				{
					OrderId = newOrderId,
					ClientId = userId,
					DishId = dishInClientCart.DishId,
					CostIncludingAppliedPromocodes = dishInClientCart
						.CostIncludingValidAppliedPromocodes
				});

			_cafeDB.AppliedPromocodesInOrders.AddRange(await (
				from appliedPromocodeInCart in _cafeDB.AppliedPromocodesInCarts
				where appliedPromocodeInCart.CartId == clientCart.CartId
				select new AppliedPromocodesInOrder()
				{
					PromocodeId = appliedPromocodeInCart.PromocodeId,
					OrderId = newOrderId,
					ClientId = userId
				}
			).ToArrayAsync());
