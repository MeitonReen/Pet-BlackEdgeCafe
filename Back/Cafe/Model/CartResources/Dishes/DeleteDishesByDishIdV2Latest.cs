...
{
	public class DeleteDishesByDishIdV2Latest : HandlerBase
	{
		...
		public DeleteDishesByDishIdV2Latest(CafeDatabase cafeDB, Guid dishId)
		{
			_cafeDB = cafeDB;
			_dishId = dishId;
		}
		...
		{
			_deletingDishesFromCart = await (
				from dishInCart in _cafeDB.DishesInCarts
				where dishInCart.DishId == deletingDishId
				select dishInCart
			).ToArrayAsync();
